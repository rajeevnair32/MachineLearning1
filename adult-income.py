...
result_dict = {
               ...
               'income - Linear SVM': build_model(linear_svm_fn, 'income', FEATURES, adult_df, options={'C': 0.1, 'loss': 'squared_hinge'}),
               'income - SVM Linear': build_model(svm_linear_fn, 'income', FEATURES, adult_df, options={'C': 0.1, 'gamma': 1}),
               'income - SVM Sigmoid': build_model(svm_sigmoid_fn, 'income', FEATURES, adult_df, options={'C': 0.1, 'gamma': 1}),
               'income - SVM RBF': build_model(svm_rbf_fn, 'income', FEATURES, adult_df, options={'C': 0.1, 'gamma': 1}),
               'income - Ada Boosting': build_model(ada_boosting_fn, 'income', FEATURES, adult_df, options={'algorithm': 'SAMME.R', 'learning_rate': 1, 'n_estimators': 100}),
               ...
               'income - Neural networks': build_model(neural_network_fn, 'income', FEATURES, adult_df, options={'activation': 'identity', 'learning_rate': 'constant', 'solver': 'lbfgs'}),
...
plt = plot_learning_curve(MLPClassifier(), 'income', FEATURES, adult_df, title, ylim=(0.4, 1.01))
#plt.show()
plt.savefig(fig_path + 'learning_curve_neural.png')

title = "Learning Curves for AdaBoost"
plt = plot_learning_curve(AdaBoostClassifier(), 'income', FEATURES, adult_df, title, ylim=(0.4, 1.01))
#plt.show()
...
plt = plot_learning_curve(SVC(), 'income', FEATURES, adult_df, title, ylim=(0.4, 1.01))
#plt.show()
plt.savefig(fig_path + 'learning_curve_svc.png')

title = "Learning Curves for LinearSVC"
plt = plot_learning_curve(LinearSVC(), 'income', FEATURES, adult_df, title, ylim=(0.4, 1.01))
#plt.show()
